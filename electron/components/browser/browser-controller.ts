...
import type { BrowserExecutionSource, BrowserOperationStatus, BrowserTraceAction, BrowserTraceItem, BrowserViewLayout, BrowserViewState, BrowserTabState } from './types'
...
import { handleDitingFileRequest } from './local-file-protocol'
...
import { hasAcknowledgedBrowserRiskDisclaimer } from './browser-settings'
import { buildDitingBrowserUserAgent } from './browser-identity'
...
export class BrowserController {
  ...
  private emit(browserSession: BrowserSessionRecord): void {
    ...
    this.owner.webContents.send('browser:state-changed', this.buildState(browserSession))
  }
  ...
  private assertRiskDisclaimerAcknowledged(): void {
    if (hasAcknowledgedBrowserRiskDisclaimer()) return
  ...
  private installPreviewProtocol(browserSession: Session): void {
    ...
    browserSession.protocol.handle('diting-file', handleDitingFileRequest)
  ...
  private createSession(sessionId: string, allowedRoots: string[] = []): BrowserSessionRecord {
    ...
    // 默认 UA 会暴露 Electron；受管网页改为诚实的 Diting 标识，并保留 Chromium token 保证站点兼容。
    browserSession.setUserAgent(buildDitingBrowserUserAgent(browserSession.getUserAgent(), app.getVersion()))
  ...
  setLayout(layout: BrowserViewLayout): void {
    ...
    if (!browserSession) return
    // Vue effect cleanup 和新 slot 的 IPC 可以交错到达。只采纳最新一代布局，
