 ...
 *
 * The renderer never receives raw diting-file:// absolute paths. Main process
...
function registerEntry(path: string, isDirectory: boolean): string {
  ...
  return `diting-file://${token}`
}

export function registerDitingFilePath(path: string): string {
  return registerEntry(path, false)
}

export function registerDitingDirectoryPath(path: string): string {
  return registerEntry(path, true)
}

export function handleDitingFileRequest(request: Request): Promise<Response> | Response {
  let url: URL
  try {
