...
import { registerDitingDirectoryPath } from './local-file-protocol'
...
/**
 * 把已授权根目录内的 HTML 文件转换为一次性/短期 diting-file 目录 URL。
...
export function createAuthorizedPreviewUrl(inputPath: string, allowedRoots: string[], baseDir?: string): { url: string; filePath: string } {
  ...
  const directoryUrl = registerDitingDirectoryPath(dirname(filePath))
...
export function isAuthorizedPreviewProtocol(url: string): boolean {
  return url.startsWith('diting-file://')
}
