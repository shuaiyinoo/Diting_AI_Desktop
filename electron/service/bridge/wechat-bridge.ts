 ...
 * - QR 码扫码登录（无需用户手动配置 Token，扫码即连接）
 * - HTTP 长轮询接收消息
 * - 发送消息/输入状态
 *
 * 用户流程：点击扫码登录 → 显示二维码 → 手机扫码 → 自动获取凭证并连接。
 * 凭证使用 safeStorage 加密保存，下次启动可直接复用。
 *
 * 消息路由到 Diting Agent，回复通过 iLink API 发送。
 */
...
export interface WeChatBridgeState {
  ...
  /** 错误信息 */
  errorMessage?: string
  /** QR 码图片内容（base64 data URL，waiting_scan 时有值） */
  qrCodeData?: string
}
...
class WeChatBridge {
  ...
  private static readonly PENDING_TTL = 10 * 60 * 1000
  ...
  private commandHandler = new BridgeCommandHandler({
    ...
    adapter: {
      sendText: async (chatId: string, text: string, meta?: unknown) => {
        ...
        for (let i = 0; i < chunks.length; i++) {
          try {
            await this.client.sendText(chatId, chunks[i]!, contextToken)
            // 多段发送时，段间加小延迟避免频率限制
            if (chunks.length > 1 && i < chunks.length - 1) {
              await sleep(500)
            }
          } catch (err) {
            logger.error(`[微信 Bridge] 发送消息第 ${i + 1}/${chunks.length} 段失败:`, redactSensitiveLogValue(err))
            // 某段失败不影响其他段，继续发送
          }
        }
      },
    ...
    bindingStore: createJsonBridgeChatBindingStore(
      join(getBridgeBindingsDir(), 'wechat-bindings.json'),
      '微信',
    ),
  ...
  async startLogin(): Promise<void> {
    // 清理现有连接，但不推送 'disconnected' 状态，避免 UI 闪烁
    ...
    try {
      ...
      logger.info('[微信 Bridge] QR 码已获取，等待扫码...')
      ...
      logger.info('[微信 Bridge] 登录成功，凭证已保存')
      ...
      logger.error('[微信 Bridge] 登录失败:', msg)
      throw error
    }
  }
      ...
      try {
        ...
        if (resp.errcode === SESSION_EXPIRED_CODE) {
          if (this.getUpdatesBuf) {
            logger.info('[微信 Bridge] session 过期，重置同步游标')
            this.getUpdatesBuf = ''
            this.saveSyncBuf()
          } else {
        ...
        if (resp.ret !== 0 && resp.errcode) {
          logger.warn('[微信 Bridge] 服务端错误:', resp.ret, resp.errcode, redactSensitiveLogText(resp.errmsg ?? ''))
          continue
        }
        ...
        for (const msg of resp.msgs) {
          ...
          try {
            ...
            logger.error('[微信 Bridge] 处理消息失败:', redactSensitiveLogValue(error))
          } finally {
        ...
        logger.warn(`[微信 Bridge] 轮询失败 (${failures}/${MAX_CONSECUTIVE_FAILURES}, backoff=${backoff}ms):`, redactSensitiveLogValue(error))

        if (failures >= MAX_CONSECUTIVE_FAILURES) {
          logger.warn('[微信 Bridge] 连续失败过多，可能需要重新登录')
        }
  ...
  private cleanExpiredPending(): void {
    ...
    for (const [chatId, entry] of this.pendingImages) {
      if (now - entry.createdAt > WeChatBridge.PENDING_TTL) {
        logger.info(`[微信 Bridge] 清理过期图片缓冲: ${chatId.slice(0, 8)}... (${entry.images.length} 张)`)
        this.pendingImages.delete(chatId)
      }
    }
    for (const [chatId, entry] of this.pendingFiles) {
      if (now - entry.createdAt > WeChatBridge.PENDING_TTL) {
        logger.info(`[微信 Bridge] 清理过期文件缓冲: ${chatId.slice(0, 8)}... (${entry.files.length} 个)`)
  ...
  private async handleMessage(msg: WeChatIncomingMessage): Promise<void> {
    ...
    logger.info('[微信 Bridge] 收到消息:', redactSensitiveLogValue({
      from: chatId,
    ...
    for (let idx = 0; idx < imageItems.length; idx++) {
      try {
        ...
        if (buf.length > MAX_IMAGE_SIZE) {
          logger.warn(`[微信 Bridge] 图片超过大小限制: ${(buf.length / 1024 / 1024).toFixed(1)}MB`)
        ...
        logger.error('[微信 Bridge] 图片下载失败:', redactSensitiveLogValue(error))
    ...
    for (let idx = 0; idx < fileItems.length; idx++) {
      ...
      if (declaredSize > MAX_FILE_SIZE) {
        logger.warn(`[微信 Bridge] 文件超过大小限制: ${(declaredSize / 1024 / 1024).toFixed(1)}MB, 文件名: ${fileName}`)
      ...
      try {
        ...
        if (buf.length > MAX_FILE_SIZE) {
          logger.warn(`[微信 Bridge] 文件实际大小超限: ${(buf.length / 1024 / 1024).toFixed(1)}MB, 文件名: ${fileName}`)
          await this.client.sendText(chatId, `⚠️ 文件「${fileName}」超过 20MB 限制，已跳过`, contextToken)
          continue
        }
        fileDownloads.push({ id: `${msgId}-file-${idx}`, data: buf, fileName })
      } catch (error) {
        logger.error(`[微信 Bridge] 文件下载失败 (${fileName}):`, redactSensitiveLogValue(error))
    ...
    if (!binding) {
      await this.client.sendText(chatId, '请先在 Diting 设置中选择 Agent 渠道。', contextToken)
      return
    }

    const workspacePath = binding.workspaceId ? getAgentWorkspacePath(binding.workspaceId) : undefined
    ...
    for (const img of allImages) {
      ...
      attachments.push({ absolutePath: path, label, kind: 'image' as const })
    }

    // 保存文件
    for (const file of allFiles) {
      const path = saveFileToSession(
        binding.workspaceId!,
        binding.sessionId,
        file.data,
        file.fileName,
      )
      attachments.push({ absolutePath: path, label: file.fileName, kind: 'file' as const })
    }
  ...
  private loadSyncBuf(): void {
    const syncState = getWeChatSyncState()
    if (syncState.getUpdatesBuf) {
      this.getUpdatesBuf = syncState.getUpdatesBuf
      logger.info('[微信 Bridge] 已加载同步游标')
    }
  }
  ...
  private updateStatus(partial: Partial<WeChatBridgeState>): void {
    ...
    for (const win of BrowserWindow.getAllWindows()) {
      if (!win.isDestroyed()) {
        win.webContents.send('controller/bridge/wechatStatusChanged', this.state)
      }
    }
  }
}
