...
description: Diting 协作子 Agent Skill。当需要并行探索多个方向（多样性探索）、对抗性审查验证已有方案、或多个长耗时独立任务需要真实可见的子会话时触发。用于判断是否以及如何调用 Diting 内置 collaboration 工具创建协作子会话。简单搜索、短调研、单文件修改、一次性代码审查由父会话直接使用普通工具完成。
group: diting
version: "1.1.1"
---

# Diting Agent Collaboration

你负责判断何时把复杂任务交给 Workflow / Skill 工作流，或拆给真实可见的 Diting 协作子 Agent 会话。

Diting 已提供内置 `collaboration` MCP 工具。你必须通过这些工具创建、等待、查看和停止协作子会话，不要用 Bash、脚本或直接修改 `~/.diting/agent-sessions.json` 的方式创建会话。
...
### Scenario 2：独立并行任务应使用 Diting 协作编排
...
Then Agent 应使用普通搜索或读文件工具，不创建真实 Diting 子会话。
