...
本技能是 `diting` CLI 的薄封装。本文件给维护者解释**底层格式**与**CLI 行为**，便于排查问题。
真正的解析逻辑在 `@diting/session-core`（仓库内唯一真源），不要在技能里重抄。

## 会话存储

```
~/.diting/agent-sessions.json        会话索引（{ version, sessions: AgentSessionMeta[] }）
~/.diting/agent-sessions/<id>.jsonl   单会话消息，JSONL（一行一条 JSON）
```

开发模式（`DITING_DEV=1` 或 Diting 未打包）数据在 `~/.diting-dev/`。CLI 用 `--dev` 或 `--config-dir` 切换。

## 两种会话格式（CLI 自动识别，无需关心）

`@diting/session-core` 的 `readSessionMessages` 在读取时统一归一，下游不需要区分：
...
```
@diting/session-core   解析 / 快照去重 / outline / search / select / render —— 真源
  └─ /node 子入口      readSessionMessages（文件 IO，含 node:fs）
apps/cli (diting)      命令路由薄壳，调用 core
default-skills/session-cleaner  本技能，教 Agent 调 CLI
```

改 bug / 改格式 → 改 `@diting/session-core`。CLI 和技能都不应包含解析逻辑。
